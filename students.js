...
//
// FIX: classId is now verified to belong to this center before use — matches
// the ownership check every other route (classes.js class-teacher, POST /
// above) already does. Previously an admin token could set a student's
// class_id to ANY class UUID, including one from a different center, which
// would then leak into results.js / reportCard.js (both compute rankings
// off class_id without re-checking center_id).
